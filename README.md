...
Las señales de entrada, **$x[n]$** y **$h[n]$**, lo hice usando los dígitos de mi cédula y el código estudiantil. La convolución, $y[n] = x[n] * h[n]$, se calculó paso a paso de forma manual, documentando cada operación.
...
Se generaron la representacion gráfica y secuenciales de la señal original ($x[n]$, $h[n]$) y de la señal resultante ($y[n]$). Este paso se realizó a mano para visualizar el desplazamiento y la superposición de las señales.
